...
namespace clinamen {
  ...
  export abstract class Node {
    ...
    tick(stack:Stack):number {
      ...
      var state:number = this.next(stack);
      if(stack.state===RUNNING){
        return stack.last().tick(stack);
      }
      if(stack.state!==RUNNING){
        stack.state = RUNNING;
      }
      while(stack.state===RUNNING){
    ...
    copy():Node{
      return this.nodeConstructor(this.copyJson) as Node;
    }

    copyJson(name:string=null):JsonData{
      var data = this.json(false);
      data._id = null;
      data.name = name || data.type;
      for(let c of this.children){
          data.push(c.copyJson());
      }
      return data;
    }
